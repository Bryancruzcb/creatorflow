...
const suite = [
  { id: 'motion', label: 'Animations', purpose: 'Read two sample GLBs: clips, skinning, morph targets.', view: 'motion' as const },
  { id: 'gpu', label: '8K image', purpose: 'Decode and draw a bundled 8192 × 8192 PNG.', view: 'gpu' as const },
  { id: 'compression', label: 'File size', purpose: 'Compare one sample scene, raw vs Draco + KTX2.', view: 'gpu' as const },
  { id: 'failures', label: 'Broken files', purpose: 'Spot six planted file problems before preview.', view: 'failures' as const },
  { id: 'package', label: 'Linked files', purpose: 'Resolve a sample glTF root, 2 buffers, 24 textures.', view: 'package' as const },
  { id: 'formats', label: 'Format samples', purpose: 'Confirm the bundled PNG, PSD, WAV, MOV, FBX, and font samples exist.', view: 'formats' as const },
] as const;
...
function stateLabel(state: ResultState) {
  ...
  if (state === 'passed') return 'Handled';
...
const dependencyFolders = [
  ...
  { id: 'environment' as const, label: 'textures/environment/', note: 'World textures' },
  { id: 'props' as const, label: 'textures/props/', note: 'Prop textures' },
];
...
function DependencyExplorer({ result }: { result: ProbeResult }) {
  ...
  return (
    <div className="dependency-explorer">
      <div className="dependency-file-tree" role="tree" aria-label="Sample project files">
        <button className={selected.path === 'world.gltf' ? 'selected' : ''} type="button" onClick={() => setSelectedPath('world.gltf')} role="treeitem" aria-selected={selected.path === 'world.gltf'}>
          <Braces size={15} /><span><strong>world.gltf</strong><small>Root file · 2.6 KB</small></span><em data-state={result.state}>{result.state === 'idle' ? 'Not checked' : result.state === 'running' ? 'Checking' : selectedResult?.ok ? 'Available' : 'Missing'}</em>
        </button>
      ...
      <aside className="dependency-file-inspector" aria-live="polite">
        <header><FileIcon role={selected.role} /><span><small>Selected file</small><strong>{selected.path.split('/').at(-1)}</strong></span></header>
        {selected.role === 'texture' ? <img src={fixtureUrl} alt={`Preview of ${selected.path}`} /> : null}
        <dl>
          <div><dt>Project path</dt><dd><code>{selected.path}</code></dd></div>
          <div><dt>Role</dt><dd>{selected.role === 'root' ? 'glTF root file' : selected.role === 'geometry' ? 'Geometry buffer' : selected.role === 'animation' ? 'Animation buffer' : 'PNG texture'}</dd></div>
          <div><dt>Size in bytes</dt><dd>{sizeOf(selected)?.toLocaleString() ?? '—'}</dd></div>
          <div><dt>Referenced by</dt><dd><code>{selected.role === 'root' ? 'System check' : 'world.gltf'}</code></dd></div>
          <div><dt>Resolution</dt><dd data-state={result.state}>{result.state === 'idle' ? 'Not checked' : result.state === 'running' ? 'Checking…' : selectedResult?.ok ? 'Inside the project folder' : 'Missing or unreachable'}</dd></div>
        ...
        <div><a className="button button-secondary" href={fixtureUrl} target="_blank" rel="noreferrer">Open file <ExternalLink size={13} /></a><button className="button button-secondary" type="button" onClick={() => { void copyPath(); }}>{copied ? <Check size={13} /> : <Copy size={13} />}{copied ? 'Copied' : 'Copy path'}</button></div>
        <p><ShieldCheck size={14} /><span><strong>Path policy</strong><small>Files must stay inside the selected folder.</small></span></p>
      </aside>
    </div>
  );
}
...
export function StressLab() {
  ...
  const systemOutcome = completedCheckCount === 0
    ? hasRunningCheck ? 'Running…' : 'Not run yet'
    : failedCount > 0
      ? 'A check failed'
      : warningCount > 0
        ? 'Handled with limits'
  ...
  async function runProbe(id: string) {
    ...
    try {
      ...
      if (id === 'motion') {
        ...
        result = { state: clips >= 6 && skins > 0 && morphTargets >= 8 ? 'passed' : 'warning', bytes: fox.bytes + morph.bytes, finding: `${clips} clips · ${skins} skin · ${morphTargets} morph targets` };
      } else if (id === 'gpu') {
        ...
        const details = Object.fromEntries(failureFixtures.map((fixture, index) => [fixture.path, { ok: detections[index], finding: detections[index] ? `Caught: ${fixture.successCopy}` : 'Not caught' }])) as Record<string, ProbeDetail>;
        result = { state: identified === failureFixtures.length ? 'passed' : 'failed', bytes: corrupt.byteLength + embedded.buffers[0].uri.length, finding: `${identified}/${failureFixtures.length} planted problems caught before preview`, details };
      } else if (id === 'package') {
        ...
        const details = Object.fromEntries([
          ['world.gltf', { ok: true, finding: 'Root file loaded', bytes: rootBytes, contentType: rootResponse.headers.get('content-type') }],
          ...checks.map((check) => [check.path, { ok: check.ok, finding: check.ok ? 'Resolved inside the project folder' : 'Missing', bytes: check.bytes, contentType: check.contentType }]),
        ...
        result = { state: availableGroups === profiles.generatedFixtures.length ? 'passed' : 'failed', bytes, finding: `${availableGroups}/${profiles.generatedFixtures.length} sample groups present · ${profiles.capacityOnly.length} capacity profiles` };
      } else throw new Error(`Unknown system check: ${id}`);
      result.durationMs = performance.now() - started;
      setResults((current) => ({ ...current, [id]: result }));
      return result;
    } catch (error) {
      const failed = { state: 'failed' as const, durationMs: performance.now() - started, finding: error instanceof Error ? error.message : 'Unknown failure' };
  ...
  const report = useMemo(() => ({
    ...
    execution: 'Browser-local checks on sample files. Does not describe your project.',
    environment: {
  ...
  const sectionNavigation = (
    <nav className="stress-tabs" aria-label="Check sections">
  ...
  return (
    <div className="stress-lab stress-clarity">
      <header className="stress-lab-header">
        <div><span>CreatorFlow device diagnostics</span><h1>Can this device run the inspection tools?</h1><p>Six sample files, to tell a browser problem from a project problem.</p></div>
        <button aria-busy={runningAll} className="button button-primary" type="button" onClick={runAll} disabled={runningAll}>{runningAll ? <RefreshCw aria-hidden="true" className="stress-spin" size={15} /> : <Zap aria-hidden="true" size={15} />} {runningAll && runningIndex >= 0 ? `Running ${runningIndex + 1}/6: ${suite[runningIndex].label}` : 'Run all 6 checks'}</button>
      </header>

      <section className="stress-verdict-scope" data-state={outcomeTone} aria-label="System check result">
        <div className="stress-verdict-primary"><span><StateMark state={outcomeTone} /></span><div><small>Result</small><strong>{systemOutcome}</strong><p>{completedCheckCount}/6 checks done, on CreatorFlow’s sample files.</p></div></div>
        <div className="stress-boundary-copy"><ShieldCheck aria-hidden="true" size={18} /><div><strong>Scope: CreatorFlow, not your Roblox project.</strong><p>Nothing here inspects, approves, blocks, publishes, or changes a release.</p></div></div>
        <dl className="stress-run-facts"><div><dt><Activity size={14} /> Local time</dt><dd>{totalDuration ? `${Math.round(totalDuration)} ms` : 'Not measured'}</dd></div><div><dt><Layers3 size={14} /> Bytes measured</dt><dd>{measuredBytes ? formatBytes(measuredBytes) : '0 B'}</dd></div><div><dt><ShieldCheck size={14} /> Uploaded</dt><dd>0 B</dd></div></dl>
      </section>

      {view === 'overview' ? (
        <>
          <section className="stress-matrix">
            <header><span>Six checks</span><strong>Known sample files, run on this device.</strong><small>Select a row for detail. Statuses are about sample files, not your project.</small></header>
            {suite.map((test, index) => {
              const result = results[test.id];
              return <button key={test.id} type="button" data-state={result.state} onClick={() => setView(test.view)} aria-label={`Open ${test.label}: ${stateLabel(result.state)}`}><span>{String(index + 1).padStart(2, '0')}</span><div className="stress-check-copy"><strong>{test.label}</strong><small>{test.purpose}</small></div><p>{result.finding ?? 'No result yet.'}</p><div className="stress-check-status"><small>Sample file</small><em className={`stress-state stress-state-${result.state}`}><StateMark state={result.state} />{stateLabel(result.state)}</em></div></button>;
            })}
          </section>
          <section className="system-check-report-entry">
            <Download size={17} />
            <div><strong>Diagnostics report</strong><small>{reportReady ? 'Ready to review or export.' : 'Run any check to create one.'}</small></div>
          ...
          <details className="system-check-guide">
            <summary><span><Gauge size={16} /><strong>How to read this</strong></span><span>3 steps <ChevronDown size={15} /></span></summary>
            <section className="system-check-purpose" aria-label="How to use the system check"><div><span>1</span><p><strong>Run the samples</strong><small>Reads about 60 MB locally. Nothing is uploaded.</small></p></div><div><span>2</span><p><strong>Open a failed check</strong><small>Its section shows what failed.</small></p></div><div><span>3</span><p><strong>Export if useful</strong><small>For comparing devices or reporting a bug.</small></p></div></section>
          </details>
          <section className="stress-detail-navigation"><header><strong>Go deeper</strong><small>The samples and measurements behind each check.</small></header>{sectionNavigation}</section>
        </>
      ) : null}
      ...
      {view === 'motion' ? (
        <section className="stress-motion-layout">
          ...
          <div className="stress-motion-detail">
            {loadedMotionId === selectedMotion.id ? <Suspense fallback={<div className="stress-load-placeholder">Loading animation…</div>}><AnimatedAssetViewer fixture={selectedMotion} onTelemetry={onMotionTelemetry} /></Suspense> : <div className="motion-gate"><img src={selectedMotion.previewUrl} alt={`${selectedMotion.name} preview`} /><div><span>Sample file · {selectedMotion.payload}</span><h2>{selectedMotion.name}</h2><p>{selectedMotion.expected}</p><button className="button button-primary" type="button" onClick={() => setLoadedMotionId(selectedMotion.id)}><Play size={15} /> Open rig and timeline</button></div></div>}
            <footer><span>{selectedMotion.license}</span><a href={selectedMotion.sourceUrl} target="_blank" rel="noreferrer">Source <ExternalLink size={13} /></a></footer>
          </div>
        </section>
      ) : null}
      ...
      {view === 'gpu' ? (
        <section className="compression-lab">
          <header><div><span>Same scene, two file sizes</span><h2>Compare download size, then decode separately</h2><p>Fetches the raw and compressed chess samples and compares bytes.</p></div><button className="button button-secondary" type="button" onClick={() => runProbe('compression')} disabled={results.compression.state === 'running'}><Activity size={15} /> Compare sizes</button></header>
          <div className="compression-ledger"><div><span>Raw GLB</span><strong>43.0 MB</strong><small>PNG/JPEG textures · uncompressed geometry</small></div><i>→</i><div><span>Draco + KTX2</span><strong>12.1 MB</strong><small>ETC1S textures · compressed geometry</small></div><div className="compression-saving"><strong>71.8%</strong><small>smaller download</small></div></div>
          <div className="gpu-pressure-row"><div><span>Real 8K sample</span><strong>8192 × 8192 PNG</strong><small>1.4 MB transfer expands to an estimated 256 MB RGBA texture before mipmaps.</small></div><button type="button" onClick={() => runProbe('gpu')}><StateMark state={results.gpu.state} /> Inspect PNG header</button></div>
          <div className="compressed-preview">
            {loadCompressed ? <Suspense fallback={<div className="stress-load-placeholder">Loading decoders…</div>}><HeavyAssetViewer url={assetUrl('/assets/beautiful-game-ktx2-draco.glb')} label="A Beautiful Game compressed" previewUrl={assetUrl('/assets/beautiful-game.jpg')} size="12.1 MB" /></Suspense> : <><img src={assetUrl('/assets/beautiful-game.jpg')} alt="Compressed chess scene preview" /><div><strong>Decoding is opt-in.</strong><span>Loading this page does not pay the decoder or 12.1 MB model cost.</span><button className="button button-primary" type="button" onClick={() => setLoadCompressed(true)}><Play size={15} /> Decode compressed scene</button></div></>}
          </div>
        </section>
      ) : null}
      ...
      {view === 'failures' ? (
        <section className="failure-lab">
          <header><div><span>Deliberately broken samples</span><h2>Does the importer catch each problem?</h2><p>These six files are broken on purpose. The results describe them, not your project, and block nothing.</p></div><button className="button button-secondary" type="button" onClick={() => runProbe('failures')} disabled={results.failures.state === 'running'}><ShieldCheck size={15} /> Check 6 broken files</button></header>
          <div className="failure-meaning"><Check size={15} /><span><strong>Green means the planted problem was caught.</strong><small>What should happen in a real project. Guidance, not a verdict.</small></span></div>
          <div>{failureFixtures.map((fixture, index) => {
            ...
            return <article key={fixture.path}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{fixture.name}</strong><code>{fixture.path}</code><small>Planted condition · {fixture.issue}</small></div><p><span>What CreatorFlow would do</span>{fixture.expectedAction}</p><em data-state={state}><StateMark state={state} />{state === 'idle' ? 'Not run' : state === 'running' ? 'Checking…' : detail?.ok ? `Caught · ${fixture.successCopy}` : 'Missed · detector needs attention'}</em></article>;
          })}</div>
        </section>
      ) : null}

      {view === 'package' ? (
        <section className="package-lab"><header><div><span>Sample project folder</span><h2>One root file, 26 linked files</h2><p>Select a file to inspect it. The scene is never rendered.</p></div><button className="button button-secondary" type="button" onClick={() => runProbe('package')} disabled={results.package.state === 'running'}><Layers3 size={15} /> Check linked files</button></header><DependencyExplorer result={results.package} /><footer><span>Path policy</span><strong>Project-relative only</strong><small><code>../outside-project.bin</code> is refused; the root file stays readable.</small></footer></section>
      ) : null}

      {view === 'formats' ? (
        <section className="format-lab"><header><div><span>Bundled samples</span><h2>Real sample files and capacity profiles</h2><p>Checks the samples are present, not that every format parses. Profiles are not bundled and never count as a pass.</p></div><button className="button button-secondary" type="button" onClick={() => runProbe('formats')}><Activity size={15} /> Check sample files</button></header><h3>Real files, bundled</h3><div className="format-executable">{executableFormats.map((fixture) => <article key={fixture.name}><span>{fixture.format}</span><strong>{fixture.name}</strong><p>{fixture.detail}</p><dl><div><dt>Payload</dt><dd>{fixture.payload}</dd></div><div><dt>Decoded</dt><dd>{fixture.decoded}</dd></div></dl></article>)}</div><h3>Capacity profiles · not bundled, never counted</h3><div className="capacity-table">{capacityProfiles.map((profile) => <div key={profile.name}><span>{profile.format}</span><strong>{profile.name}</strong><small>{profile.detail}</small><em>{profile.payload}</em></div>)}</div></section>
      ) : null}

      {view === 'telemetry' ? (
        <section className="telemetry-lab"><header><div><span>Diagnostics report</span><h2>What ran, and what this device returned</h2><p>For comparing devices or reporting a bug. Sample files only — no verdict about your project.</p></div><button className="button button-secondary" type="button" onClick={downloadReport} disabled={!reportReady}><Download size={15} /> Export JSON</button></header><div className="telemetry-table"><div className="telemetry-head"><span>Check</span><span>Result</span><span>Local time</span><span>Bytes measured</span><span>Finding</span></div>{suite.map((test) => { const result = results[test.id]; return <div key={test.id}><strong>{test.label}</strong><span className={`stress-state stress-state-${result.state}`}><StateMark state={result.state} />{stateLabel(result.state)}</span><code>{result.durationMs !== undefined ? `${result.durationMs.toFixed(1)} ms` : '—'}</code><code>{result.bytes ? formatBytes(result.bytes) : '—'}</code><p>{result.finding ?? 'Not run'}</p></div>; })}</div>{Object.keys(motionTelemetry).length ? <div className="motion-telemetry"><h3>Live viewer measurements</h3>{Object.entries(motionTelemetry).map(([id, telemetry]) => <div key={id}><strong>{motionFixtures.find((fixture) => fixture.id === id)?.name ?? id}</strong><dl><div><dt>Decode</dt><dd>{telemetry.decodeMs.toFixed(1)} ms</dd></div><div><dt>First render</dt><dd>{telemetry.firstRenderMs.toFixed(1)} ms</dd></div><div><dt>Draw calls</dt><dd>{telemetry.drawCalls}</dd></div><div><dt>Triangles</dt><dd>{telemetry.triangles.toLocaleString()}</dd></div><div><dt>Materials</dt><dd>{telemetry.materials}</dd></div><div><dt>Textures</dt><dd>{telemetry.textures}</dd></div></dl></div>)}</div> : null}<div className="telemetry-disclosure"><AlertTriangle size={16} /><p><strong>Not measured</strong>Not Roblox runtime performance, not exact GPU memory, not your project. Capacity profiles never pass.</p></div></section>
      ) : null}
    </div>
  );
}
